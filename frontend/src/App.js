...
const DropdownPortal = ({ isOpen, coords, onClose, children }) => {
  if (!isOpen) return null;
  return createPortal(
    <div 
...
function ChatInterface() {
  ...
  // ── Sprinkle ✨ Feedback Particle Effect ──
  const triggerSprinkle = (e, emoji) => {
    const burstCount = 7; // Precisely 7 as requested
    const container = document.body;
    
    // Get mouse/button position for origin
    ...
    for (let i = 0; i < burstCount; i++) {
      ...
      el.innerText = emoji;
      el.className = 'sprinkle-emoji';
      
      // Random trajectory "all over the page"
      const vx = (Math.random() - 0.5) * 1000; // Large spread
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      // 💎 Fix: removed setMenuOpenId(null) from mousedown.
      // This was closing the menu before the onClick could fire.
      // The DropdownPortal overlay handles closure via its own onClick.
  ...
  const askAI = async (customQuestion, showInChat = true) => {
    ...
    // 💎 Only add user message to state if showInChat is true
    if (showInChat) {
    ...
    try {
      ...
      setSessions(prev => prev.map(s => {
        if (s.id === activeSessionId) {
          const isFirstMessage = s.messages.length === 0 || (s.messages.length === 1 && showInChat);
  ...
  const handleLogout = () => {
    // 🛡️ Complete State Reset for Data Isolation
    setSessions([]);
    setActiveSessionId("");
    setFiles([]);
    setIsUploaded(false);
        ...
        {/* 📂 Document Controls (Now in Sidebar) */}
          ...
          <div className="upload-card-compact">
            <input 
              ...
              onChange={(e) => { setFiles(Array.from(e.target.files)); setIsUploaded(false); }} 
            />
            ...
            <div key={s.id} className={`history-item ${activeSessionId === s.id ? 'active' : ''}`} onClick={() => setActiveSessionId(s.id)}>
              {editingSessionId === s.id ? (
                ...
                <button 
                  ...
                  onClick={(e) => { 
                    e.stopPropagation(); 
                    console.log("Three-dot menu clicked for session:", s.id);
                    const rect = e.currentTarget.getBoundingClientRect(); // 💎 Use currentTarget for absolute accuracy
                    setMenuCoords({ x: rect.left - 160, y: rect.top + 30 }); 
              ...
              >
                <div 
                  ...
                  onClick={(e) => { 
                    e.preventDefault();
                    e.stopPropagation(); 
                    console.log("Rename clicked for session:", s.id);
                ...
                <div 
                  ...
                  onClick={(e) => { 
                    e.preventDefault();
                    e.stopPropagation(); 
                    console.log("Pin/Unpin clicked for session:", s.id);
                    setSessions(prev => prev.map(item => item.id === s.id ? { ...item, isPinned: !item.isPinned } : item)); 
                    setMenuOpenId(null); 
                  }}
                >
                ...
                <div 
                  ...
                  onClick={(e) => { 
                    e.preventDefault();
                    e.stopPropagation(); 
                    console.log("Delete clicked for session:", s.id);
                    setSessions(sessions.filter(item => item.id !== s.id)); 
                    if (activeSessionId === s.id) setActiveSessionId(""); 
                    setMenuOpenId(null); 
                  }}
                >
                  🗑️ Delete
        ...
        <div className="chat-content-container">
          <header className="chat-header-compact">
            <h1 className="brand-title">
              Insight<span className="brand-accent">Lens</span>
            </h1>
            ...
            <div className={`quick-actions-bar ${!isUploaded ? 'locked' : ''}`}>
              <button 
                onClick={() => {
                  console.log("Summarize All clicked");
                  askAI("Summarize all uploaded documents in 3-5 concise sentences.", false);
                }} 
                disabled={!isUploaded}
              >
                📄 Summarize All
              ...
              <button 
                onClick={() => {
                  console.log("Key Insights clicked");
                  askAI("What are the most important key points aggregated from these documents?", false);
                }} 
                disabled={!isUploaded}
              >
                📍 Key Insights
              ...
              <button 
                onClick={() => {
                  console.log("Advanced Analysis clicked");
                  askAI("What are the deepest AI-generated opportunities mentioned across these files?", false);
                }} 
                disabled={!isUploaded}
              >
                🔍 Advanced Analysis
                ...
                <div className="empty-state">
                  <div className="empty-state-hero">
                    <div className="hero-overlay">
                      <p>Welcome to the future of document analysis. Select PDFs in the sidebar to begin. ✨</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <footer className="chat-footer">
            <div className="input-box">
              <div className="plus-menu-container" ref={plusMenuRef}>
                <button 
                  className={`plus-button ${isPlusMenuOpen ? 'active' : ''}`} 
                ...
                {isPlusMenuOpen && (
                  <div className="plus-popup-menu">
                    <div className="plus-menu-item" onClick={() => { fileInputRef.current.click(); setIsPlusMenuOpen(false); }}>
                      <span className="menu-item-icon">📄</span>
                      <span className="menu-item-text">Upload PDF</span>
                    </div>
                  </div>
                )}
              </div>
